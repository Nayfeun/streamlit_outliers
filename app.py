...
def distribution_graph(distribution, formula: Formula):
    ...
    threshold = get_threshold(distribution_ndarray, formula.mad_weight, formula.iqr_weight, formula.sd_weight, formula.mad_constant,
...
def formula_choice(custom: bool) -> Formula:
    ...
    if not custom:
        ...
        user_formula.mad_weight, user_formula.iqr_weight, user_formula.sd_weight = [
            st.number_input(label=f'{method} weight (%)', min_value=0, max_value=100, step=1)/100 for method in Formula.METHODS]

        user_formula.mad_constant, user_formula.iqr_constant, user_formula.sd_constant = [
            st.number_input(label=f'{method} constant', min_value=1.0, max_value=5.0, step=0.5) for method in Formula.METHODS]
