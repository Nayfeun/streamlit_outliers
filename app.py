...
file_path_distributions = os.path.join("src", "distributions.csv")
file_path_outliers = os.path.join("src", "outliers.csv")
...
def formula_choice(custom):
    if not custom:
        outlier_method = st.radio("Method", ['2.5 MAD', '1.5 IQR', '2.5 SD', '3.0 SD'])
        if outlier_method.endswith('SD'):
            user_mad_weight, user_iqr_weight, user_sd_weight = 0, 0, 1
            user_mad_constant, user_iqr_constant, user_sd_constant = 0, 0, float(outlier_method[:3])
        elif outlier_method.endswith('MAD'):
            user_mad_weight, user_iqr_weight, user_sd_weight = 1, 0, 0
            user_mad_constant, user_iqr_constant, user_sd_constant = float(outlier_method[:3]), 0, 0
        else:
            user_mad_weight, user_iqr_weight, user_sd_weight = 0, 1, 0
            user_mad_constant, user_iqr_constant, user_sd_constant = 0, 1.5, 0
    ...
    return user_mad_weight, user_iqr_weight, user_sd_weight, user_mad_constant, user_iqr_constant, user_sd_constant
...
with tab1:
    ...
    user_distribution_shape = st.radio("Distribution shape : ",
                                       [distribution_shape.title() for distribution_shape in distributions_shapes])

    user_outliers_shape = st.radio("Outliers shape: ",
                                   [outliers_shape.title().replace("_", " ") for outliers_shape in outliers_shapes])

    user_distribution_size = st.slider(label="Distribution size", min_value=1, max_value=1000)
    user_outliers_rate = st.slider(label="Outliers rate (%)", min_value=1, max_value=40) / 100
...
user_mad_weight, user_iqr_weight, user_sd_weight, user_mad_constant, user_iqr_constant, user_sd_constant = \
    formula_choice(custom)
